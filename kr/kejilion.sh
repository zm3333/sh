...
cf_purge_cache() {
  ...
  if [ -f "$CONFIG_FILE" ]; then
	...
	if [[ "$answer" == "y" ]]; then
	  echo "CF 정보는 다음 위치에 저장됩니다.$CONFIG_FILE，可以后期修改CF信息"
...
dd_xitong() {
		...
		dd_xitong_1() {
		  echo -e "重装后初始用户名: ${gl_huang}root${gl_bai}  初始密码: ${gl_huang}LeitboGi0ro${gl_bai}초기 포트:${gl_huang}22${gl_bai}"
		...
		dd_xitong_2() {
		  echo -e "重装后初始用户名: ${gl_huang}Administrator${gl_bai}초기 비밀번호:${gl_huang}Teddysun.com${gl_bai}  初始端口: ${gl_huang}3389${gl_bai}"
		  echo -e "계속하려면 아무 키나 누르세요..."
		  read -n 1 -s -r -p ""
		  install wget
		  dd_xitong_MollyLau
		...
		dd_xitong_4() {
		  echo -e "재설치 후 초기 사용자 이름:${gl_huang}Administrator${gl_bai}  初始密码: ${gl_huang}123@@@${gl_bai}초기 포트:${gl_huang}3389${gl_bai}"
		  echo -e "계속하려면 아무 키나 누르세요..."
		  read -n 1 -s -r -p ""
		  dd_xitong_bin456789
		}
...
linux_tools() {
  ...
  while true; do
	  ...
	  echo -e "${gl_kjlan}3.   ${gl_bai}sudo 超级管理权限工具             ${gl_kjlan}4.   ${gl_bai}socat 통신 연결 도구"
...
linux_ldnmp() {
  while true; do
	...
	echo -e "${gl_huang}------------------------"
	echo -e "${gl_huang}35.  ${gl_bai}LDNMP 환경 보호${gl_huang}36.  ${gl_bai}优化LDNMP环境"
	...
	while true; do
		...
		case "$Limiting" in
			...
			22) # 解压文件/目录
				...
				install tar
				tar -xzvf "$filename" && echo "압축이 풀렸습니다.$filename" || echo "解压失败"
				send_stats "解压文件/目录"
				;;
...
while true; do
	  ...
	  echo -e "${gl_kjlan}11. ${gl_bai}기술 사자 스크립트 설치${gl_kjlan}12. ${gl_bai}更新系统              ${gl_kjlan}13. ${gl_bai}시스템 청소"
