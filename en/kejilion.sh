...
linux_panel() {
	...
	while true; do
	  ...
	  echo -e "${gl_kjlan}13.  ${gl_bai}Cloudreve network disk${gl_huang}★${gl_bai}                     ${gl_kjlan}14.  ${gl_bai}简单图床图片管理程序"
	  ...
	  echo -e "${gl_kjlan}------------------------"
	  echo -e "${gl_kjlan}21.  ${gl_bai}VScode web version${gl_kjlan}22.  ${gl_bai}UptimeKuma监控工具"
	  ...
	  echo -e "${gl_kjlan}63.  ${gl_bai}OpenWebUI自托管AI平台 ${gl_huang}★${gl_bai}             ${gl_kjlan}64.  ${gl_bai}it-tools toolbox"
	  ...
	  echo -e "${gl_kjlan}69.  ${gl_bai}SFTPGo文件传输工具                  ${gl_kjlan}70.  ${gl_bai}AstrBot chatbot framework"
	  echo -e "${gl_kjlan}------------------------"
...
linux_Settings() {
	...
	while true; do
	  ...
	  echo -e "${gl_kjlan}3.   ${gl_bai}ROOT password login mode${gl_kjlan}4.   ${gl_bai}安装Python指定版本"
	...
	while true; do
		...
		case "$Limiting" in
			...
			13) # 修改文件权限
				...
				read -e -p "请输入权限 (如 755): " perm
				chmod "$perm" "$filename" && echo "权限已修改" || echo "Modification failed"
				send_stats "修改文件权限"
				;;
			14) # 重命名文件
			...
			15) # 删除文件
				...
				rm -f "$filename" && echo "File deleted" || echo "删除失败"
				send_stats "删除文件"
				;;
			21) # 压缩文件/目录
