...
clamav() {
		  ...
		  while true; do
				...
				echo "------------------------"
				echo -e "${gl_lv}1. Full scan${gl_bai}             ${gl_huang}2. 重要目录扫描 ${gl_bai}            ${gl_kjlan} 3. 自定义目录扫描 ${gl_bai}"
				echo "------------------------"
				echo "0. 返回上一级选单"
				echo "------------------------"
				read -e -p "Please enter your choice:" sub_choice
...
delete_task_schedule() {
	...
	echo "Task number deleted$num 的定时任务"
}
...
linux_tools() {
  ...
  while true; do
	  ...
	  echo -e "${gl_kjlan}7.   ${gl_bai}unzip ZIP压缩解压工具             ${gl_kjlan}8.   ${gl_bai}tar GZ compression and decompression tool"
...
linux_panel() {
	...
	while true; do
	  ...
	  echo -e "${gl_kjlan}------------------------"
	  echo -e "${gl_kjlan}1.   ${gl_bai}宝塔面板官方版                      ${gl_kjlan}2.   ${gl_bai}aaPanel Pagoda International Version"
	  echo -e "${gl_kjlan}3.   ${gl_bai}1Panel new generation management panel${gl_kjlan}4.   ${gl_bai}NginxProxyManager visualization panel"
	  echo -e "${gl_kjlan}5.   ${gl_bai}OpenList多存储文件列表程序          ${gl_kjlan}6.   ${gl_bai}Ubuntu Remote Desktop Web Edition"
	  ...
	  echo -e "${gl_kjlan}15.  ${gl_bai}emby多媒体管理系统                  ${gl_kjlan}16.  ${gl_bai}Speedtest speed test panel"
	  echo -e "${gl_kjlan}17.  ${gl_bai}AdGuardHome removes adware${gl_kjlan}18.  ${gl_bai}onlyofficeOnline office OFFICE"
	  echo -e "${gl_kjlan}19.  ${gl_bai}Leichi WAF firewall panel${gl_kjlan}20.  ${gl_bai}portainer容器管理面板"
	  echo -e "${gl_kjlan}------------------------"
	  echo -e "${gl_kjlan}21.  ${gl_bai}VScode web version${gl_kjlan}22.  ${gl_bai}UptimeKuma monitoring tool"
	  echo -e "${gl_kjlan}23.  ${gl_bai}Memos网页备忘录                     ${gl_kjlan}24.  ${gl_bai}Webtop remote desktop web version${gl_huang}★${gl_bai}"
	  ...
	  case $sub_choice in
		  ...
		  77)
			...
			docker_rum() {
				...
				read -e -p "设置${docker_name}Login password:" app_passwd
...
echo -e "科技lion周边"
echo "------------------------"
echo -e "${gl_kjlan}Station B:${gl_bai}https://b23.tv/2mqnQyh              ${gl_kjlan}油管: ${gl_bai}https://www.youtube.com/@kejilion${gl_bai}"
...
echo -e "Command line input${gl_huang}k${gl_kjlan}可快速启动脚本${gl_bai}"
echo -e "${gl_kjlan}------------------------${gl_bai}"
