...
manage_country_rules() {
	...
	case "$action" in
		...
		allow)
			...
			echo "Successfully allowed only$country_code 的 IP 地址"
			rm "${country_code,,}.zone"
			;;
...
ldnmp_web_status() {
	...
	while true; do
		...
		echo "站点目录"
		echo "------------------------"
		echo -e "数据 ${gl_hui}/home/web/html${gl_bai}Certificate${gl_hui}/home/web/certs${gl_bai}     配置 ${gl_hui}/home/web/conf.d${gl_bai}"
		echo "------------------------"
		echo ""
		echo "操作"
		echo "------------------------"
...
linux_tools() {
  ...
  while true; do
	  ...
	  echo -e "${gl_kjlan}------------------------"
	  echo -e "${gl_kjlan}41.  ${gl_bai}Install specified tools${gl_kjlan}42.  ${gl_bai}卸载指定工具"
	  echo -e "${gl_kjlan}------------------------"
...
linux_work() {
	...
	while true; do
	  ...
	  echo -e "${gl_huang}提示: ${gl_bai}After entering the workspace, use Ctrl+b and then press d alone to exit the workspace!"
	  echo -e "${gl_kjlan}------------------------"
...
linux_Settings() {
	...
	while true; do
	  ...
	  echo -e "${gl_kjlan}23.  ${gl_bai}限流自动关机                       ${gl_kjlan}24.  ${gl_bai}ROOT private key login mode"
...
while true; do
	  ...
	  case $sub_choice in
		  ...
		  5)
			  ...
			  echo -e "请上传您的 ${gl_huang}servers.py${gl_bai}file to${gl_huang}/root/cluster/${gl_bai} 完成还原！"
			  break_end
			  ;;
